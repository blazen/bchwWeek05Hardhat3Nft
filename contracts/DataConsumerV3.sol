...
contract DataConsumerV3 {
   ...
   */
  function getChainlinkDataFeedLatestAnswer(uint256 feedType) public view returns (uint256) {
   ...
   /**
    ...
    function ethPrice() external view returns (uint256) {
        // amount is in smallest units; convert to USD using price decimals.
        uint256 price = getChainlinkDataFeedLatestAnswer(999);
    ...
    function usdcPrice() external view returns (uint256) {
        // amount is in smallest units; convert to USD using price decimals.
        uint256 price = getChainlinkDataFeedLatestAnswer(1);
